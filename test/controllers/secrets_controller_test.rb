...
class SecretsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create secret" do
    assert_difference('Secret.count') do
      post secrets_url, params: { secret: { address: @secret.address, city: @secret.city, doc_name: @secret.doc_name, doc_value: @secret.doc_value, due_date: @secret.due_date, name: @secret.name, neighborhood: @secret.neighborhood, plan: @secret.plan, secret: @secret.secret, secret_password: @secret.secret_password, state: @secret.state, wireless_password: @secret.wireless_password, wireless_ssid: @secret.wireless_ssid } }
    end
